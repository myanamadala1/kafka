...
if [ "${type}" == "binary" ]; then
  ...
  curl -L -k -X POST -H "Content-Type: application/vnd.kafka.${type}.v2+json" -H "Accept: application/vnd.kafka.v2+json" --data "${data}" "${KAFKA_REST_BASE_URL}/topics/${KAFKA_TOPIC}"
  echo -e "\n"
  sleep 1
done < "$input"
